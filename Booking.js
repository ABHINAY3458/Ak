...
const Booking =()=>{
   ...
   const[CarModel,setCarType]=React.useState('');
   const[MobileNo,setMobileNo]=React.useState('');
   const Booking= async()=>{
      console.warn(Name,Email,CarModel,MobileNo);
      alert("Your car has been booked") 
      const userId = JSON.parse( localStorage.getItem('user'))._id;
      let result =await fetch("http://localhost:5000/add-booking",{
         method:'post',
         body:JSON.stringify({Name,Email,CarModel,MobileNo,userId}),
         headers:{
   ...
   return(
    ...
    <div className="carbookings">
                        ...
                        <input type="text"  placeholder="Enter your Car Model" className="inputBox"
                        onChange={(e)=>{setCarType(e.target.value)}} value={CarModel}/>
